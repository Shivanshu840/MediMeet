...
export default function HealthMonitor() {
  ...
  const fetchHealthData = async () => {
    try {
      ...
      if (data.data) {
        setWeight(data.data.weight || weight);
        setFoodCalories(data.data.foodCalories || foodCalories);
        setSteps(data.data.steps || steps);
        setHeartRate(data.data.heartRate || heartRate);
        setSleepTime(data.data.sleepTime || sleepTime);
        setBloodPressure(data.data.bloodPressure || bloodPressure);
        setTemperature(data.data.temperature || temperature);
        if (data.data.airQuality) {
          setAirQuality(data.data.airQuality);
        }
      }
    } catch (error) {
    ...
    return (
      <div className="relative h-[200px] mt-4">
        <div className="absolute inset-0 flex items-center justify-center">
          <Heart
            className={`h-32 w-32 text-red-500 transition-transform duration-200 ${
              rate > 100 ? "scale-110" : "scale-100"
            }`}
          />
        </div>
        <svg className="w-full h-full" viewBox="0 0 400 100">
          <path
            d={`M0,50 Q100,<span class="math-inline">\{50 \- rate / 2\} 200,50 T400,</span>{50 + rate / 2}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
  ...
  const analyzeHealthData = async () => {
    ...
    try {
      ...
      const result = await response.json();
      console.log("Health data analyzed successfully", result);

      toast({
        title: "Health analysis complete",
        description:
          "Your health data has been analyzed. Check the insights panel for recommendations.",
      });
    } catch (error) {
    ...
    <div className="space-y-6">
        ...
        <Card className="p-6 bg-zinc-900/80 backdrop-blur-sm border-zinc-800 col-span-1 lg:col-span-2">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {heartRate} <span className="text-sm text-zinc-400">bpm</span>
              </p>
            </div>
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 mt-8">
        <Button
          ...
          disabled={isUpdating}
        >
          {isUpdating ? "Updating..." : "Update Health Data"}
        </Button>

        <Button
          onClick={analyzeHealthData}
          className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 text-lg font-semibold rounded-lg transition-colors duration-200 flex items-center justify-center"
