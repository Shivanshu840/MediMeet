...
export default function AppointmentsPage() {
  ...
  const handleCancel = async () => {
    if (appointmentToCancel) {
      try {
        ...
        setMessage({
          type: "success",
          text: "Your appointment has been successfully canceled and removed.",
        });
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-zinc-900 via-zinc-800/90 to-emerald-900/40 flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-900 via-zinc-800/90 to-emerald-900/40 py-12">
      <div className="container mx-auto p-4 max-w-4xl">
        {message && (
          <div
            className={`mb-4 p-4 rounded-md ${message.type === "success" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
          >
            {message.text}
          </div>
        )}
                      ...
                      <DialogContent className="bg-zinc-900 border border-zinc-800">
                        <DialogHeader>
                          <DialogTitle className="text-white">
                            Payment for Dr. {appointment.doctor.lastName}
                          </DialogTitle>
                          <DialogDescription className="text-zinc-400">
                            Choose your payment method for the appointment fee
                            of ${appointment.fee}
                          </DialogDescription>
                        </DialogHeader>
                        <RadioGroup
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem
                              ...
                              className="border-emerald-500 text-emerald-400"
                            />
                            <Label htmlFor="paypal" className="text-zinc-300">
                              PayPal
                            </Label>
                          </div>
                        </RadioGroup>
                    ...
                    <DialogContent className="bg-zinc-900 border border-zinc-800">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-zinc-400">
                          ...
                          Dr. {appointment.doctor.lastName} on{" "}
                          {new Date(appointment.dateTime).toLocaleDateString()}{" "}
                          at{" "}
                          {new Date(appointment.dateTime).toLocaleTimeString()}?
                        </DialogDescription>
